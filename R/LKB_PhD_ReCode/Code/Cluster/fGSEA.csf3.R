...
setorder(res, padj)
res[,leadingEdge:=sapply(res$leadingEdge, paste0, collapse=";")] #tidies leadingEdge column, though re-separation will require SplitDataTableWithMultiRows() or strsplit()

#OutFolder <- paste0(getwd(), "/", StartTime, "FGSEA")
#if(!dir.exists(OutFolder)) { dir.create(OutFolder) }
#FileName <- paste0(OutFolder, "/FGSEA-", CONF_N_REPS, "-", PathwayFile, "-Results")
FileName <- paste0(StartTime, "_FGSEA-", CONF_N_REPS, "-", PathwayFile, "-Results.tsv")
write.table(res, FileName, sep="\t", col.names=TRUE, row.names=FALSE, quote=FALSE)
message("Saved to tsv file.")
